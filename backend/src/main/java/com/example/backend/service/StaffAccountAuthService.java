...
import com.example.backend.dto.request.StaffAuthRequest;
...
@Service
public class StaffAccountAuthService {
    ...
    public StaffAccountAuthService(StaffAccountRepository staffAccountRepository, InvalidJwtTokenRepository invalidJwtTokenRepository, RefreshTokenRepository refreshTokenRepository, PasswordEncoder passwordEncoder, StaffAccountMapper staffAccountMapper) {
    ...
    // public AuthenticationResponse authenticate(StaffAuthRequest staffAuthRequest, String clientIp, String userAgent) {
    //     StaffAccount staffAccount = staffAccountRepository.findByUsername(staffAuthRequest.getUsername()).orElseThrow(() -> new AppException(ErrorCode.STAFFACCOUNT_NOTEXISTED));
    //     boolean isAuthenticated = passwordEncoder.matches(staffAuthRequest.getPassword(), staffAccount.getPassword());
    //     if (!isAuthenticated) {
